 ...
 * Find two borders 0 <= l1 <= l2 <= l-1, such that the block starting at column
 * l1 and ending at column l2 consists only of reads with mean quality score at
 * least m. Here it is allowed to ignore arbitrary reads. Determine l1, l2 and
 * the selected reads in such a way, that the area
 *           (width*height = (l2 - l1 - 1) * (number of selected reads)         
 * is maximized. If the option for the output file is given, a CSV is written
 * with the number of selected reads for all given pairs of left and right
 * borders.
 *
 * RUNTIME: O( l^2 * n )
 *
...
int main(int argc, char * argv[]) {
    ...
    try{
        ...
        int** c = trimIntegerMean(inputFile, numberOfSequences, lengthOfSequence, givenMinMean, shift);
